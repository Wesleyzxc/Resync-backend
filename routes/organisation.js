...
router.post("/org/add", function (req, res, next) {
  ...
  if (org_name == undefined || owner == undefined) {
    return res.status(400).send({ error: "Missing organisation name or owner name" });
  }
  req.jwt.verify(token, process.env.secret, function (err) {
    if (err) {
      return res.status(401).send({ error: "Invalid token" });
...
router.put("/org/update", function (req, res, next) {
  ...
  let org_name = req.headers.organisation_name;
  if (org_name == undefined) return res.status(400).send({ message: "No organisation provided to update." });
  ...
  let state = req.headers.state;
  let country = req.headers.country;
  if (new_OrgName == undefined && newOwner == undefined && address == undefined && city == undefined && state == undefined && country == undefined)
  ...
  req.jwt.verify(token, process.env.secret, function (err) {
    if (err) {
      ...
      req
        ...
        .catch((err) => {
          console.log(err);
          return res.status(400).send({ message: "The database cannot update this change." });
        });
    }
  });
});
